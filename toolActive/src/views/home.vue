        ...
        <van-popup v-model="popupShow" position="bottom" class="popup">
                ...
                <div class="act-info">
                    <div class="text-info">
                        ...
                        <div class="row">
                            ...
                            <span class="text2 sm">{{ activeInfo.startTime |  timestamp(activeInfo.endTime) }}</span>
                        </div>
                        <div class="row">
                        ...
                        <div class="row">
                            <p class="text1">主办方</p>
                            <span class="text2">{{activeInfo.organizer}}</span>
                        </div>
                        <div class="row">
                            <p class="text1">活动人数</p>
                            <span class="text2">{{activeInfo.joinNumber}}/{{maxJoinCount}}</span>
                        </div>
                    </div>
                    <div class="act-img">
            ...
            <div class="act-sign-up" @click="codeShow = !codeShow">
                ...
                <div v-show="codeShow" class="dialog">
                    <img src="../assets/images/big-code.png" alt="" />
                    <p>扫描二维码报名</p>
                </div>
            </div>
        </van-popup>
...
export default {
    ...
    created() {
        ...
        tagReq.append("fidEnc", "e4466e1bcd4c3a57")
        ...
        const urlParams = new URLSearchParams(document.location.hash.substring(3))
        this.fidEnc = urlParams.get("fidEnc")
    },
    computed:{
        maxJoinCount(){
            return this.activeInfo.maxJoinCount === 0 ? '不限' : this.activeInfo.maxJoinCount
        }
    },
    filters: {
        timestamp: function(start, end) {
            let s = new Date(start)
            let s1 = s.toLocaleDateString()
            let e = new Date(end)
            let e1 = e.toLocaleDateString()
            if (s1 === e1) {
                return (
                    s1.replace(/\//g, ".") +
                    " " +
